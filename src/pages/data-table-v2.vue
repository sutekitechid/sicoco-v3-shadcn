	...
	<div class="p-4">
		...
		<div class="mb-8">
			...
			<div class="mb-4 flex gap-2">
				<Button @click="openAllRoleDetails">Open all modules</Button>
				<Button @click="closeAllRoleDetails">Close all modules</Button>
			</div>
			<DataTable
				...
				scroll-y="''"
			>
				<DataTableColumn field="menu" :order="1">
					<template #header><span class="block text-left">Menu / Submenu</span></template>
					<template #default="{ row }">
						<div class="flex items-center text-left">
							...
							>
								<template #icon-left>
								<i class="si-heroicon-outline-minus shrink-0 text-secondary" />
								</template>	
							</SButton>
							<div class="flex flex-col justify-center gap-1">
								<span>{{ row.menu }}</span>
								<span v-if="row.description" class="text-body-md text-secondary">
									{{ row.description }}
								</span>
							</div>
						</div>
				...
				<DataTableColumn field="view" :order="2" width="8rem" >
					<template #header>Lihat</template>
					<template #default="{ row }">
						<div class="flex justify-start">
							<Checkbox
								v-if="isActionAvailable(row, 'view')"
				...
				<DataTableColumn field="create" :order="3" width="8rem">
					<template #header>Tambah</template>
					<template #default="{ row }">
						
						<div class="flex justify-start">
							<Checkbox
								v-if="isActionAvailable(row, 'create')"
				...
				<DataTableColumn field="update" :order="4" width="8rem">
					<template #header>Ubah</template>
					<template #default="{ row }">
						<div class="flex justify-start">
							<Checkbox
								v-if="isActionAvailable(row, 'update')"
				...
				<DataTableColumn field="delete" :order="5" width="8rem">
					<template #header>Hapus</template>
					<template #default="{ row }">
						
						<div class="flex justify-start">
							<Checkbox
								v-if="isActionAvailable(row, 'delete')"
...
const roleDataTableRef = ref(null)
const openedDetailedRoles = ref(['pengendalian'])
const selectedPermissions = ref({
	beranda: ['view', 'create'],
	evaluasi: ['view', 'create', 'update'],
	'verifikasi-auditor': ['view', 'create', 'update'],
	'verifikasi-dokumen': ['view'],
	peningkatan: ['view'],
	repositori: ['view', 'create', 'delete'],
})
const roleData = [
	{
		id: 'beranda',
	...
	},
	{
		...
		children: [
			{
				id: 'verifikasi-auditor',
				description: 'Verifikasi oleh Auditor',
				actions: ['view', 'create', 'update'],
			},
		],
	},
...
function openAllRoleDetails() {
	roleDataTableRef.value?.openAllDetails()
}

function closeAllRoleDetails() {
	roleDataTableRef.value?.closeAllDetails()
}
...
function isActionAvailable(row, action) {
