...
export function useSelectRow(props, filteredData, emit) {
	...
	const computedIsRowSelectable = computed(() => {
		return filteredData.value.map(row => props.isRowSelectable(row))
	})

	const selectableRows = computed(() => {
		return filteredData.value.filter(row => props.isRowSelectable(row))
	})
