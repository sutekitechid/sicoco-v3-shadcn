...
export function useDataTableScrollSync() {
	...
	function syncHorizontalScrollFromFooter(dataTableScrollWrapper, tableVirtualWrapper, scrollLeft) {
		...
		if (tableVirtualWrapper && tableVirtualWrapper.scrollLeft !== scrollLeft) {
			tableVirtualWrapper.scrollLeft = scrollLeft
		}
	}
	 ...
	 * @param {Function} handleInfiniteScrollDebounced - Debounced infinite scroll handler
	 * @param {Object} props - Component props
	 * @param {HTMLElement} tableVirtualWrapper - Virtual scroll wrapper element
	 * @param {Object} footerScrollWrapper - Footer scroll wrapper ref
	 */
	function onScrollEvent(event, handleInfiniteScrollDebounced, props, tableVirtualWrapper, footerScrollWrapper) {
		...
		syncHorizontalScrollToVirtual(tableVirtualWrapper, event.target.scrollLeft)
		syncHorizontalScrollToFooter(footerScrollWrapper, event.target.scrollLeft)
	}
	...
	function setupScrollSynchronization(dataTableScrollWrapper, tableVirtualWrapper, footerScrollWrapper, syncFunctions) {
		...
		const throttledSyncToFooter = useThrottleFn((scrollLeft) => {
			syncFunctions.syncHorizontalScrollToFooter(scrollLeft)
		}, 0) // ~60fps
