...
export function useTreeOperations() {
  ...
  const filterTreeByVisibility = (tree, isColumnVisible) => {
    const filteredTree = tree.map(node => filterNodeByVisibility(node, isColumnVisible)).filter(Boolean)
    
    // Add right border to the last node in the root level
    if (filteredTree.length > 0) {
      const lastNode = filteredTree[filteredTree.length - 1]
      filteredTree[filteredTree.length - 1] = addRightBorderToLastLeaf(lastNode)
    }
    
    return filteredTree
  }

  const addRightBorderToLastLeaf = (node) => {
    if (!node) return node
    
    // Ensure hasBorderRight is applied to the rightmost node at its level, either directly or recursively through its children
    let updatedNode = { ...node, hasBorderRight: true }
    
    // If it's a leaf node, return it with the right border
    if (node.isLeaf) {
      return updatedNode
    }
    
    // If it has children, recursively add right border to the last child
    if (node.children && node.children.length > 0) {
      const updatedChildren = [...node.children]
      const lastChildIndex = updatedChildren.length - 1
      updatedChildren[lastChildIndex] = addRightBorderToLastLeaf(updatedChildren[lastChildIndex])
  ...
  const filterNodeByVisibility = (node, isColumnVisible, hasParent, index) => {
    if (!node) return null
    ...
    if (!node.isLeaf && node.children.length > 0) {
      ...
      const filteredChildren = node.children
        .map((child, childIndex) => filterNodeByVisibility(child, isColumnVisible, true, childIndex))
        .filter(Boolean)
