...
export function useTreeOperations() {
  ...
  const flattenTreeToRows = (tree, depth = null) => {
    ...
    function walk(nodes, level, hasBorderRight = false) {
      ...
      nodes.forEach((node, index) => {
        ...
        const col = {
          ...
          hasBorderRight: node.hasBorderRight || index === nodes.length - 1 ? mHasBorderRight : false,
    ...
    /** Add border right to last node in the first level/root
     * The border right is added if the node has subheader and the next node does not have a subheader,
     * because the header without subheader does not have a left border.
    */
    tree.forEach((node, index) => {
      node.hasBorderRight = checkHasBorderRight(node, tree[index + 1])
    })

    function checkHasBorderRight(node, nextNode) {
      let nextNodeHasSubheader = nextNode && nextNode.hasSubheader
      return Boolean(node.hasSubheader && !node.group && !nextNodeHasSubheader)
    }
