 ...
 */
export function useDataTablePinning(props, filteredData, allLeafColumns, groups, tree) {
	// ============================
	// REACTIVE STATE
	// ============================
	...
	function getActualColumnWidth(columnId) {
		...
		return TABLE_CONSTANTS.DEFAULT_COLUMN_WIDTH // fallback width
	}
	...
	function isValidForPinning(column) {
		...
		return Boolean(column.field || column.name)
	}
	...
	/**
	 * Process columns and groups with pin props to automatically add them to pinning arrays
	 */
	...
	function restorePinningStateFromLocalStorage() {
		...
		try {
			...
			// After restoring from localStorage, process columns with pin props
			// This ensures columns with pin props are added to the arrays
			processPinPropsInNextTick()
		} catch (error) {
			console.warn('Failed to restore pinning state:', error)
			pinnedLeft.value = []
			pinnedRight.value = []
			
			// Even if localStorage restore fails, still process pin props
			processPinPropsInNextTick()
		}
	}

	function processPinPropsInNextTick() {
		// Use nextTick to ensure DOM is ready before processing pin props
		nextTick(() => {
	...
	watch(
		[() => filteredData.value, pinnedLeft, pinnedRight, allLeafColumns],
		() => {
			updateStickyOffsets()
		},
