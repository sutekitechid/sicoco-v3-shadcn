...
export function useDataTablePinning(props, allLeafColumns, groups, tree) {
	...
	/**
	 * Generate object berisi index/posisi setiap field/group dari tree
	 * @returns {Object} - { [fieldOrGroupName]: index }
	 */
	...
	/**
	 * Insert field/group name to pinned array sorted by columnPositions order
	 * @param {Array} pinnedArray - pinnedLeft.value atau pinnedRight.value
	 * @param {string} fieldOrGroupName - nama field/group yang akan ditambahkan
	 * @param {Object} columnPositions - hasil dari computed columnPositions
	 */
	function insertPinnedSorted(pinnedArray, fieldOrGroupName, columnPositions) {
		if (pinnedArray.includes(fieldOrGroupName)) return

		// Ambil semua field/group yang sudah ada + yang baru
		const allPinned = [...pinnedArray, fieldOrGroupName]

		// Urutkan berdasarkan posisi 'left' di columnPositions
		allPinned.sort((a, b) => {
