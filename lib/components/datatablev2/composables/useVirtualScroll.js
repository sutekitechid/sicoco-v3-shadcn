 ...
 */
export function useVirtualScroll(props, filteredData, sortedNodes, treeOps, rowSize) {
	// ============================
	// REACTIVE STATE
	// ============================
	...
	const shouldUseVirtualScroll = computed(() => {
		...
		const hasData = filteredData.value && filteredData.value.length > 0
	...
	function getRowHeight(index) {
		...
		if (filteredData.value && filteredData.value[index]) {
			const row = filteredData.value[index]
	...
	// Returns all visible leaf columns with cell info but WITHOUT applying colspan/rowspan merging.
	// Used by DataTableDummyBody so it always renders one cell per column for accurate width measurement.
	function getBaseRowColumns() {
		const leafColumns = treeOps.collectLeafColumns(sortedNodes.value)
	...
	watch(() => filteredData.value, clearRowspanTracker, { deep: true })
