...
export function registerValidateFunc(func: any, slotValidateFuncList: any) {
  ...
  const funcIndex = slotValidateFuncList.value.findIndex((item: { id: string }) => item.id === func.id)
...
export const removeValidateFunc = (id: string, slotValidateFuncList: any) => {
  const funcIndex = slotValidateFuncList.value.findIndex((item: { id: string }) => item.id === id)
  if (funcIndex !== -1) {
...
export function validate(slotValidateFuncList: any, emit: any) {
  let focused = false
  let valid = true
  slotValidateFuncList.value.forEach((item: { id: string, validate: () => boolean, focusFunction?: () => void, reset: () => void }) => {
