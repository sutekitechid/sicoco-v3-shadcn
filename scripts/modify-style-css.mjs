...
const fontSourcePattern =
	/src:\s*url\(\s*(['"])(?:data:font\/[^'"]+|fonts\/suteki-icon\.woff(?:\?[^'"]+)?|woff\/suteki-icon\.woff)\1\s*\)(?:\s*format\([^)]*\))?/
...
css = css.replace(fontSourcePattern, 'src: url("woff/suteki-icon.woff") format("woff")')
