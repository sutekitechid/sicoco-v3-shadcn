...
test('Button as-child applies button classes to the child root', () => {
  const wrapper = mount(Button, {
    props: {
      asChild: true,
    },
    slots: {
      default: () => h('a', { href: '#as-child' }, 'As child'),
    },
  })

  expect(wrapper.element.tagName).toBe('A')
  expect(wrapper.classes()).toContain('hover:bg-primary-hover')
})
...
const BASE =
  'inline-flex items-center justify-center gap-2 whitespace-nowrap ' +
	...
	'[&:not([disabled])]:active:scale-[0.98] disabled:cursor-not-allowed ' +
